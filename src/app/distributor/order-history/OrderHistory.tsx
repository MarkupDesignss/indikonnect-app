...
const ModalShell = ({ onClose, maxWidth = "max-w-xl", children }: ModalShellProps) => (
    <AnimatePresence>
        <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
...
const TRACKING_STEPS: Array<{ key: TimelineKey; label: string; icon: any; group: "forward" | "return" }> = [
    ...
    { key: "return_completed_at", label: "Return Completed", icon: Package, group: "return" },
];
...
const TrackingModal = ({ isOpen, onClose, order }: TrackingModalProps) => {
    ...
    const isRejected = !!timeline.return_rejected_at;
        ...
        <ModalShell onClose={onClose} maxWidth="max-w-lg">
            <div className="flex shrink-0 items-center justify-between border-b border-[#E6E6E4] px-5 py-4 sm:px-6">
                <div className="flex items-center gap-2">
                    <div className="flex h-8 w-8 items-center justify-center rounded-[6px]" style={{ backgroundColor: "#eaf7f0" }}>
                        <Truck className="h-4 w-4" style={{ color: EMERALD }} />
                    </div>
                    <div>
                        <h3 className="text-[15px] font-semibold text-[#171717] sm:text-[16px]">Track Order</h3>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-6">
                <div className="mb-6 flex items-center gap-3 rounded-[7px] border border-[#E4E4E2] bg-[#FAFAF9] p-3.5">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-0.5 text-[10px] text-[#888888]">Qty: {order.quantity}</p>
                        {lastStep && (
                            <p className="mt-1 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold" style={{ color: EMERALD, backgroundColor: "#eaf7f0" }}>
                ...
                {steps.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-10 text-center">
                        ...
                        <p className="mt-3 text-[12px] text-[#999999]">Tracking details will appear here once your order is processed.</p>
                    </div>
                ) : (
                    <div>
                        {steps.map((step, i) => {
                            ...
                            const dotColor = isReturnRejected ? RED : step.group === "return" ? BRASS : EMERALD;
                            const dotBg = isReturnRejected ? "#FEF2F2" : step.group === "return" ? "#F8F1E4" : "#eaf7f0";
                            const Icon = step.icon;
                            return (
                                <div key={step.key} className={`relative pl-11 ${isLast ? "" : "pb-7"}`}>
                                    {!isLast && (
                                        <div
                                            className="absolute left-[15px] top-8 w-[2px] rounded-full"
                                            style={{ height: "calc(100% - 1.75rem)", backgroundColor: "#E4E4E2" }}
                                        />
                                    )}
                                    <div
                                    ...
                                    <div className="pt-0.5">
                                        <p className="text-[12.5px] font-semibold text-[#171717]">{step.label}</p>
                                        <p className="mt-0.5 text-[11px] text-[#888888]">{formatDate(timeline[step.key])}</p>
                                    </div>
                                </div>
                            );
                        })}
...
const OrderBreakupModal = ({ isOpen, onClose, order }: BreakupModalProps) => {
    ...
    const rows: Array<{ label: string; value: string; muted?: boolean }> = [
        { label: "Subtotal", value: formatCurrency(summary.subtotal || order.subtotal) },
        { label: "Total GST", value: formatCurrency(summary.total_tax || order.total_gst) },
        { label: "Shipping", value: formatCurrency(summary.shipping_charge || order.shipping_charge) },
        {
            label: "Coin Redeemed",
    ...
    return (
        <ModalShell onClose={onClose} maxWidth="max-w-md">
            <div className="flex shrink-0 items-center justify-between border-b border-[#E6E6E4] px-5 py-4 sm:px-6">
                <div className="flex items-center gap-2">
                    <div className="flex h-8 w-8 items-center justify-center rounded-[6px]" style={{ backgroundColor: "#f8f1e4" }}>
                        <Receipt className="h-4 w-4" style={{ color: BRASS }} />
                    </div>
                    <div>
                        <h3 className="text-[15px] font-semibold text-[#171717] sm:text-[16px]">Price Breakup</h3>
                        <p className="mt-0.5 text-[10px] text-[#888888] sm:text-[11px]">{order.order_reference}</p>
                    </div>
                </div>
                <button
                    onClick={onClose}
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-6">
                <div className="rounded-[10px] border border-dashed border-[#DADADA] bg-[#FAFAF9] p-4">
                    <div className="space-y-3">
                        {rows.map((row) => (
                            <div key={row.label} className="flex items-center justify-between">
                                <span className={`text-[12px] ${row.muted ? "text-[#999999]" : "text-[#555555]"}`}>{row.label}</span>
                                <span className={`text-[12.5px] font-medium ${row.muted ? "text-[#999999]" : "text-[#171717]"}`}>
                                    {row.value}
                                </span>
                            </div>
                        ))}
                    ...
                    <div className="flex items-center justify-between rounded-[8px] px-3 py-2.5" style={{ backgroundColor: NAVY }}>
                        <span className="text-[12.5px] font-semibold text-white">Grand Total</span>
                ...
                <div className="mt-4 rounded-[10px] border border-[#E4E4E2] bg-white p-4">
                    <h4 className="mb-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#888888]">Payment</h4>
                    <div className="space-y-2.5 text-[12px]">
                        <div className="flex items-center justify-between">
                            ...
                            <span
                                ...
                                style={{
                                    ...
                                    backgroundColor: order.payment_status === "paid" ? "#eaf7f0" : "#f8f1e4",
                                }}
                            >
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <span className="font-medium capitalize text-[#171717]">{order.payment_gateway || "—"}</span>
                        </div>
                        {order.gateway_transaction_id && (
                            <div className="flex items-center justify-between gap-3">
                                <span className="flex-shrink-0 text-[#888888]">Transaction ID</span>
                                <span className="truncate font-medium text-[#171717]" title={order.gateway_transaction_id}>
                                    {order.gateway_transaction_id}
                                </span>
                            </div>
                        )}
...
const OrderDetails = ({ order, onTrack, onViewBreakup }: OrderDetailsProps) => {
    ...
    return (
        ...
        >
            <div className="grid gap-5 px-6 py-5 md:grid-cols-2 xl:grid-cols-3">
                {/* Order Summary — condensed, full breakup in a popup */}
                <div className="rounded-[10px] border border-[#e7e9ee] bg-white p-4">
                    ...
                    <div className="rounded-[8px] px-3.5 py-3" style={{ backgroundColor: NAVY }}>
                        <div className="flex items-center justify-between">
                    ...
                    <div className="mt-1.5 flex items-center justify-between text-[11.5px] text-[#667085]">
                        <span>Gateway</span>
                        <span className="font-medium capitalize text-[#101828]">{order.payment_gateway}</span>
                    </div>
                    <button
                ...
                {/* Tracking — condensed, full stepper in a popup */}
                <div className="rounded-[10px] border border-[#e7e9ee] bg-white p-4">
                    <h4 className="mb-3 flex items-center gap-1.5 text-[12px] font-bold uppercase tracking-wider text-[#667085]">
                    ...
                    {latestStep ? (
                        <div className="flex items-center gap-3 rounded-[8px] border border-[#eaf7f0] bg-[#eaf7f0] px-3.5 py-3">
                            <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-white">
                                <latestStep.icon className="h-4 w-4" style={{ color: EMERALD }} />
                            </div>
                            <div className="min-w-0">
                                <p className="text-[12.5px] font-semibold text-[#101828]">{latestStep.label}</p>
                                <p className="text-[10.5px] text-[#667085]">{formatShortDate(timeline[latestStep.key])}</p>
                            </div>
                        </div>
                    ) : (
                ...
                {returns.length > 0 && (
                    <div className="rounded-[10px] border border-[#e7e9ee] bg-white p-4 md:col-span-2 xl:col-span-1">
                        ...
                        {returns.map((ret: any, i: number) => (
                            <div key={i} className="mb-3 rounded-[6px] border border-[#eaecf0] bg-[#fafbfc] p-3 text-[11.5px] last:mb-0">
                                ...
                                <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-[11px] text-[#667085]">
                                    ...
                                    <span className="text-right font-medium">{formatCurrency(ret.refund_subtotal)}</span>
                                    <span>Refund Tax:</span>
                                    <span className="text-right font-medium">{formatCurrency(ret.refund_tax)}</span>
                                    <span>Refund Total:</span>
                                    <span className="text-right font-medium">{formatCurrency(ret.total_refund_amount)}</span>
                                    {ret.refund_status && (
                                        <>
                                            <span>Refund Status:</span>
                                            <span className="text-right font-medium capitalize">{ret.refund_status}</span>
                                        </>
                                    )}
                                    {ret.admin_notes && (
                                        <>
                                    ...
                                    {ret.rejection_reason && (
                                        <>
                                            <span className="text-[#DC2626]">Rejection Reason:</span>
                                            <span className="text-right text-[#DC2626]">{ret.rejection_reason}</span>
                                        </>
                                    )}
                                </div>
                            </div>
                    ...
                    <div className="rounded-[10px] border border-[#e7e9ee] bg-white p-4 md:col-span-2 xl:col-span-1">
                        ...
                        <div className="space-y-2">
                            {creditNotes.map((cn: any, i: number) => (
                                <div key={i} className="rounded-[6px] bg-[#f7f8fa] p-2.5 text-[11.5px]">
                    ...
                    <div>
                        {filteredOrders.length === 0 ? (
                            ...
                            filteredOrders.map((order, idx) => {
                                ...
                                return (
                                    <div key={rowKey} className="border-b border-dashed border-[#e7e9ee] last:border-b-0">
