...
export default function ProductsPage(): JSX.Element {
  ...
  const brandIdMap = useMemo(() => {
    ...
    categoriesData?.brands?.forEach((brand: any) => {
      if (brand?.id != null) {
        map.set(String(brand.id), Number(brand.id));
      }
    });
    ...
    categoriesData?.data?.forEach((category: Category) => {
      (category.subcategories || []).forEach((subCategory) => {
        if (subCategory?.id != null) {
          map.set(String(subCategory.id), subCategory);
        }
      });
    });
  ...
  const handlePageChange = useCallback(
    (page: number) => {
      if (page < 1) return;
      if (lastPage > 0 && page > lastPage) return;
  ...
  const paginationPages = useMemo(() => {
    ...
    if (currentPage <= 3) {
      for (let i = 1; i <= VISIBLE_PAGES; i++) pages.push(i);
      return pages;
    }
  ...
  const renderPagination = () => {
    if (isLoading || lastPage <= 1 || totalProducts <= 0) {
      return null;
    }
          ...
          <aside className="hidden md:block">
            <div className="sticky top-24">
              <div
                ...
                style={{ fontFamily: "Lato, sans-serif" }}
              >
                <span className="text-[#7d8aa0]">Home</span>
                <ChevronRight className="h-3 w-3 text-[#adb4be]" />
                <span className="font-medium text-[#111111]">
                  {isNewArrivals ? "New Arrivals" : "Collection"}
                </span>
              </div>
