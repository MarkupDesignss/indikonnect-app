...
const getProductMrp = (product: any, accountType?: string): number => {
  ...
  return Number(
    product.retail_mrp ?? product.retail?.mrp ?? product.mrp ?? 0,
  );
};
...
export default function ProductsPage() {
  ...
  const { hasToken, userType: storedUserType } = useTokenCheck();

  // ✅ Guest ke liye profile API skip
  const { data: userProfile } = useGetUserProfileQuery(
    {},
    { skip: hasToken !== true },
  );

  // ✅ Guest → retail, Logged-in → profile based
  const userType = useMemo(() => {
    if (hasToken !== true) return "retail";
    if (userProfile) return getAccountType(userProfile);
    return storedUserType === "distributor" ? "distributor" : "retail";
  }, [hasToken, userProfile, storedUserType]);
  ...
  const getInitialPriceRange = (): [number, number] => {
    ...
    return [
      ...
      Number.isFinite(max) && max > 0 ? max : MAX_PRICE_LIMIT,
    ];
  };
  ...
  /* ===================================================
     CATEGORY API (public — guest bhi chalega)
  =================================================== */
  ...
  const categoryIdMap = useMemo(() => {
    ...
    categoriesData?.data?.forEach((cat: Category) => {
      ...
      if (title && cat?.id != null) map.set(title, Number(cat.id));
    });
    return map;
  }, [categoriesData]);
  ...
  const brandIdMap = useMemo(() => {
    const map = new Map<string, number>();
    categoriesData?.brands?.forEach((brand: any) => {
      if (brand?.id != null) map.set(String(brand.id), Number(brand.id));
    });
    return map;
  }, [categoriesData]);
  ...
  useEffect(() => {
    ...
    const urlBrands = brandParam
      ? brandParam.split(",").map((item) => item.trim()).filter(Boolean)
      : [];
    ...
    const subCategoryParam = searchParams.get("subcategory_ids");
    const urlSubCategoryIds = subCategoryParam
      ? subCategoryParam.split(",").map((item) => item.trim()).filter(Boolean)
      : [];

    const validSubCategoryIds = categoriesData?.data?.length
    ...
    setFilters((prev) => {
      ...
      return JSON.stringify(prev) === JSON.stringify(nextFilters)
        ? prev
        : nextFilters;
    });
  ...
  }, [searchParamsString, categoriesData]);
  ...
  const updateBrowserUrl = useCallback(
    ({
      ...
      nextSearch = searchQueryRef.current,
    }: {
      ...
      if (isNewArrivals) params.set("new-arrivals", "true");

      if (nextFilters.brands.length > 0)
        params.set("brand_ids", nextFilters.brands.join(","));
      ...
      if (nextFilters.availability.inStock) params.set("in_stock", "true");
  ...
  const queryParams = useMemo(() => {
    ...
    if (isNewArrivals) params.new_arrivals = true;

    if (filters.brands.length > 0) {
      const brandIds = filters.brands
        .map((id) => brandIdMap.get(String(id)))
    ...
    if (filters.subCategories.length > 0) {
      const subCategoryIds = filters.subCategories
        ...
        .join(",");
      if (subCategoryIds) params.subcategory_ids = subCategoryIds;
    }
  ...
  /* ===================================================
     PRODUCTS API (public — guest bhi chalega)
  =================================================== */

  const { data: productsData, isLoading, isFetching, error, refetch } =
  ...
  const paginationPages = useMemo(() => {
    ...
    pages.push(1);
    if (rangeStart > 2) pages.push(-1); // ellipsis marker
    for (let i = rangeStart; i <= rangeEnd; i++) pages.push(i);
    if (rangeEnd < total - 1) pages.push(-2); // ellipsis marker
    if (total > 1) pages.push(total);

    return pages;
  }, [lastPage, apiCurrentPage]);
  ...
  useEffect(() => {
    setActiveBanner(0);
  }, [currentPage, brandBanners.length]);
  ...
  useEffect(() => {
    if (!isLoading && lastPage > 0 && currentPage > lastPage) {
      const validPage = lastPage;
      setCurrentPage(validPage);
      updateBrowserUrl({ nextPage: validPage });
    }
  }, [isLoading, lastPage, currentPage, updateBrowserUrl]);
  ...
  useEffect(() => {
    if (!transformedProducts.length) {
      if (!isFetching && !isLoading) setAllProducts([]);
      return;
    }

    setAllProducts((prev) => {
      if (apiCurrentPage === 1) return transformedProducts;

      const existingIds = new Set(prev.map((p) => p.id));
      const newItems = transformedProducts.filter(
        (p) => !existingIds.has(p.id),
      );

      return [...prev, ...newItems];
    });
  }, [transformedProducts, apiCurrentPage, isFetching, isLoading]);
  ...
  const filterSignature = useMemo(
    ...
    [filters, sortBy, searchQuery, isNewArrivals],
  );

  const prevFilterSignatureRef = useRef<string>(filterSignature);

  useEffect(() => {
    if (prevFilterSignatureRef.current === filterSignature) return;
  ...
  useEffect(() => {
    if (!isFetching && !isLoading && showFilterLoader) {
      setShowFilterLoader(false);
    }
  }, [isFetching, isLoading, showFilterLoader]);
  ...
  }, [isLoading, isFetching, currentPage, lastPage, allProducts.length]);
  ...
  const hasMoreStateRef = useRef<boolean>(hasMore);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (isLoading || isFetching) return;
        if (hasMoreStateRef.current) return;
    ...
    observer.observe(node);
    return () => observer.disconnect();
  }, [isLoading, isFetching, allProducts.length, hasMore]);
  ...
  const handleFilterChange = useCallback(
    (newFilters: FilterState) => {
      ...
      setShowEndMessage(false);
      setShowFilterLoader(true);
      updateBrowserUrl({ nextFilters: newFilters, nextPage: 1 });
  ...
  const handleSearch = useCallback(
    (query: string) => {
      ...
      setShowEndMessage(false);
      setShowFilterLoader(true);
      updateBrowserUrl({ nextSearch: query, nextPage: 1 });
  ...
  const handleSortChange = useCallback(
    (sort: SortOption) => {
      ...
      setShowEndMessage(false);
      setShowFilterLoader(true);
      updateBrowserUrl({ nextSort: sort, nextPage: 1 });
  ...
  const handlePageChange = useCallback(
    (page: number) => {
      ...
      setShowEndMessage(false);
      setShowFilterLoader(true);
      updateBrowserUrl({ nextPage: page });
  ...
  const pageTitle = isNewArrivals ? "New Arrivals" : "Collection";
  ...
  const isFilterLoading = showFilterLoader;

  const isAppending = isFetching && allProducts.length > 0;
  ...
  const renderPagination = () => {
    if (isLoading || lastPage <= 1 || totalProducts <= 0) return null;

    const currentPageNum = apiCurrentPage || currentPage;
        ...
        <nav className="flex items-center gap-1.5" aria-label="Pagination">
          ...
          {paginationPages.map((page) =>
            page < 0 ? (
              <span
                key={`ellipsis-${page}`}
                className="flex h-8 min-w-[32px] items-center justify-center text-[11px] text-[#999999]"
              >
                …
              </span>
            ) : (
              <button
                ...
                className={`flex h-8 min-w-[32px] items-center justify-center rounded-lg border px-2 text-[11px] font-medium transition ${currentPageNum === page
    ...
    >
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-[250px_minmax(0,1fr)] lg:grid-cols-[264px_minmax(0,1fr)] lg:gap-8">
          ...
          <main ref={productsSectionRef} className="min-w-0 scroll-mt-[90px]">
            {/* ITEMS COUNT + SEARCH */}
                ...
                {isInitialLoading || isFilterLoading ? (
                  ...
                  <span>
                    <span className="font-medium">
                      {totalProducts.toLocaleString("en-IN")} items
                    </span>
                    <span className="mx-1.5 text-[#c5c8cd]">|</span>
                    <span>Curated selections for you</span>
                  </span>
                )}
              </div>
            ...
            {/* MOBILE SORT */}
            <div className="mb-4 md:hidden">{renderSortSelect()}</div>
              ...
              <div className="flex max-w-[560px] items-center gap-3 rounded-lg bg-gradient-to-r from-[#ffe2b8] via-[#fff1dc] to-white px-4 py-2.5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-[#5e5e5e]">
                    Quick dispatch on all orders, delivered securely to your
                    doorstep
                  </p>
                </div>
              </div>
            ...
            {showInitialSkeleton ? (
              ...
              <>
                <div
                  className={`grid grid-cols-2 gap-x-3 gap-y-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:gap-x-4 xl:gap-y-6 transition-opacity duration-200 ${isFetching && !isAppending
                    ? "opacity-60"
                    : "opacity-100"
                    }`}
                >
                  {products.map((product: any, index: number) => {
                    ...
                    return (
                      <div
                        ...
                        className="min-w-0"
                      >
                        {/* ✅ hasToken pass kiya — ProductCard guest ke liye login redirect karega */}
                        <ProductCard
                          product={product}
                ...
                {/* END SENTINEL */}
                <div ref={endSentinelRef} className="h-4 w-full" />

                {/* APPENDING LOADER */}
                {isAppending && renderBrandLoader()}

                {/* END MESSAGE */}
                {showEndMessage && !hasMore && products.length > 0 && (
