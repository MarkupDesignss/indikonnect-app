...
import { FormActions } from "@/components/Distributor/Register/components/FormActions";
import { StepProps } from "@/components/Distributor/Register/types";
...
export const ReviewStep: React.FC<StepProps> = ({
  ...
  const clearAllDataAndRedirectToHome = () => {
    // Clear all distributor registration data from localStorage
    const registrationKeys = [
      ...
      "distributor_aadhaar_data",
      "distributor_pan_data",
      "distributor_bank_data",
      "distributor_location_data",
      "distributor_sponsor_data",
      "distributor_terms_accepted",
      "distributor_agreement_accepted",
      "distributor_code_of_conduct_accepted",
    ];
    ...
    // Redirect to home page with full history clear
    // This replaces the entire history so user cannot go back to registration
    router.replace("/");

    // OR if you want to add to history but clear all previous entries:
    // router.push("/");

    // OR for external domain:
    // window.location.replace("https://roman.com/");
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.status) {
        ...
        // Store application data in localStorage for reference
        ...
        // Clear all data and redirect to home after 2 seconds
        setTimeout(() => {
          clearAllDataAndRedirectToHome();
        }, 2000);
      } else {
        // Submission failed
        const errorMsg =
