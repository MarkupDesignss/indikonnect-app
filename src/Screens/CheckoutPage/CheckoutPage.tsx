...
import { useGetCheckoutSummaryQuery } from "@/lib/redux/api/checkoutApi";
              ...
              {taxBreakdown.length > 0 && (
                <div className="bg-gradient-to-br from-[#FBF6EC] to-[#F5EDE0] rounded-xl p-4 border border-[#E7DBC0]">
                  ...
                  <div className="space-y-1.5">
                    {taxBreakdown.map((tax: any, index: number) => (
                      <motion.div 
                        key={index}
                      ...
                      >
                        ...
                        <span className="text-[#2B2420] font-medium">
                          {formatPrice(tax.cgst + tax.sgst + tax.igst)}
                        </span>
                      </motion.div>
                    ))}
                  </div>
                  ...
                  {taxBreakdown.map((tax: any, index: number) => (
                    ...
                    >
                      <span className="text-[#8a7f6e]">
                        {tax.tax_category?.toUpperCase()} ({tax.rate})
                      </span>
                      <span className="text-[#2B2420]">
                        {formatPrice(tax.cgst + tax.sgst + tax.igst)}
                      </span>
    ...
    <div className="space-y-4">
      <div className="bg-gradient-to-br from-[#FBF6EC] to-[#F5EDE0] rounded-xl p-4 border border-[#E7DBC0]">
        <div className="flex items-center gap-2 mb-3">
          <div className="w-7 h-7 rounded-full bg-gradient-to-r from-[#C9A227] to-[#E5C84A] flex items-center justify-center">
            <CreditCard className="w-3.5 h-3.5 text-white" />
          </div>
          <span className="text-sm font-medium text-[#2B2420]">Payment Method</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <motion.div
            ...
            className={`p-3 bg-white rounded-lg border-2 transition-all cursor-pointer ${
              selectedPayment === "card" 
                ? "border-[#C9A227] shadow-md shadow-[#C9A227]/20" 
                : "border-[#E7DBC0] hover:border-[#C9A227]/40"
            }`}
          >
            <div className="flex items-center gap-2">
              <div className="w-8 h-5 bg-gradient-to-r from-blue-600 to-blue-400 rounded shadow-sm"></div>
              <p className="text-sm font-medium text-[#2B2420]">Credit Card</p>
            </div>
            <p className="text-xs text-[#8a7f6e] mt-1">•••• •••• •••• 4242</p>
            {selectedPayment === "card" && (
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
          ...
          >
            <div className="flex items-center gap-2">
              <div className="w-8 h-5 bg-gradient-to-r from-blue-500 to-blue-300 rounded shadow-sm"></div>
              <p className="text-sm font-medium text-[#2B2420]">PayPal</p>
            </div>
            <p className="text-xs text-[#8a7f6e] mt-1">paypal@email.com</p>
            {selectedPayment === "paypal" && (
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
      ...
      <motion.button
        ...
        className="w-full py-3.5 bg-gradient-to-r from-[#2B2420] via-[#3d322b] to-[#2B2420] text-white rounded-xl font-medium hover:from-[#C9A227] hover:via-[#D4A92E] hover:to-[#E5C84A] transition-all duration-500 disabled:opacity-60 disabled:cursor-not-allowed shadow-lg shadow-[#2B2420]/20 flex items-center justify-center gap-2 relative overflow-hidden group"
      >
        {isSubmitting ? (
          <>
            <motion.div
...
export default function CheckoutPage() {
  ...
  const handleCheckout = () => {
    if (!selectedDeliveryAddress) {
      dispatch(showToast({ message: "Please select a delivery address to continue", type: "error" }));
      return;
    }
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p
                                  className={`text-sm font-bold mt-1.5 ${
                                    priceValue === 0 ? "text-green-700" : "text-[#2B2420]"
                                  }`}
                                >
                                  {price}
                                </p>
                ...
                >
                  ...
                  <div className="p-5">
                    <PaymentStep
                      ...
                      selectedBillingAddress={useSameBilling ? selectedDeliveryAddress : selectedBillingAddress}
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <motion.button
                    ...
                    className="flex items-center gap-2 px-8 py-3 rounded-full bg-gradient-to-r from-[#2B2420] via-[#3d322b] to-[#2B2420] text-white text-sm font-semibold hover:from-[#C9A227] hover:via-[#D4A92E] hover:to-[#E5C84A] transition-all duration-500 disabled:opacity-60 disabled:cursor-not-allowed shadow-md shadow-[#2B2420]/20 group"
                  >
                    {isSubmitting ? (
                      <>
                        <motion.div
