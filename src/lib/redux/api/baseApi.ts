...
> = async (args, api, extraOptions) => {
  let result = await baseQuery(args, api, extraOptions); // If unauthorized, try to refresh token
      ...
      try {
        ...
        if (refreshResult.data) {
          ...
          if (data.status && data.access_token) {
            // Store new tokens
            TokenManager.setTokens(data.access_token, data.refresh_token); // Retry the original request with new token
