...
const modalVariants = {
  ...
  exit: { scale: 0.9, opacity: 0, y: 20, transition: { duration: 0.18 } }
};
...
const PasswordChange = ({ isOpen, onClose, onSave }) => {
  ...
  const fields = [
    ...
    { key: 'new', label: 'New Password', field: 'newPassword', placeholder: 'Enter new password' },
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-gray-800 font-bold text-lg">Change Password</h2>
              </div>
              <motion.button
            ...
            >
              {fields.map(({ key, label, field, placeholder }) => (
              ...
              <motion.div variants={staggerItem} className="flex gap-3 pt-2">
                ...
                <motion.button
                  whileHover={{ scale: 1.02, boxShadow: '0 8px 20px rgba(23,31,57,0.3)' }}
                  whileTap={{ scale: 0.97 }}
...
const Notifications = ({ isOpen, onClose, onSave }) => {
  ...
  const notificationOptions = [
    ...
    { key: 'pushNotifications', label: 'Push Notifications', icon: Bell, description: 'Receive push notifications' },
    { key: 'orderUpdates', label: 'Order Updates', icon: BellRing, description: 'Get notified about order status' },
    { key: 'paymentAlerts', label: 'Payment Alerts', icon: Shield, description: 'Payment confirmation and alerts' },
    { key: 'promotionalEmails', label: 'Promotional Emails', icon: Mail, description: 'Special offers and promotions' },
    { key: 'securityAlerts', label: 'Security Alerts', icon: Shield, description: 'Important security notifications' },
    { key: 'newsletter', label: 'Newsletter', icon: Mail, description: 'Monthly updates and news' }
  ];
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-gray-800 font-bold text-lg">Notification Settings</h2>
              </div>
              <motion.button
                whileHover={{ scale: 1.15, rotate: 90 }}
                whileTap={{ scale: 0.9 }}
                  ...
                  return (
                    <motion.div
                      ...
                      className={`p-4 rounded-xl border-2 transition-colors cursor-pointer ${
                        isEnabled ? 'border-[#171f39] bg-[#171f39]/5' : 'border-gray-200 hover:border-gray-300'
                      }`}
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <motion.div
                          animate={{
                            backgroundColor: isEnabled ? '#171f39' : '#F3F4F6',
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between">
                            ...
                            <motion.div
                              animate={{ backgroundColor: isEnabled ? '#171f39' : '#D1D5DB' }}
                              transition={{ duration: 0.25 }}
...
const EditProfilePopup = ({ isOpen, onClose, formData, setFormData, onSave }) => {
  ...
  const handleImageUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        setLocalFormData((prev) => ({ ...prev, profile_picture: reader.result }));
      };
  ...
  const handleRemoveImage = () => {
    ...
    setLocalFormData((prev) => ({ ...prev, profile_picture: null }));
  };
  ...
  const getPopupFields = () => {
    const commonFields = [
      ...
      { name: 'phone', label: 'Phone Number', type: 'text', icon: Smartphone, placeholder: 'Enter phone number', readOnly: true },
      { name: 'email', label: 'Email Address', type: 'email', icon: Mail, placeholder: 'Enter email address', readOnly: true },
      { name: 'country', label: 'Country', type: 'text', icon: User, placeholder: 'Enter your country' }
    ];

    if (localFormData.account_type === 'distributor') {
      return [
        ...commonFields,
        { name: 'bank_holder_name', label: 'Bank Account Holder', type: 'text', icon: User, placeholder: 'Enter account holder name' },
        { name: 'encrypted_bank_account', label: 'Bank Account Number', type: 'text', icon: CreditCard, placeholder: 'Enter account number' },
        { name: 'bank_ifsc', label: 'IFSC Code', type: 'text', icon: Hash, placeholder: 'Enter IFSC code' },
        { name: 'encrypted_pan', label: 'PAN Number', type: 'text', icon: FileText, placeholder: 'Enter PAN number' },
        { name: 'encrypted_aadhaar', label: 'Aadhaar Number', type: 'text', icon: FileText, placeholder: 'Enter Aadhaar number' }
      ];
    }
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-gray-800 font-bold text-lg">Edit Profile</h2>
              </div>
              <motion.button
                whileHover={{ scale: 1.15, rotate: 90 }}
                whileTap={{ scale: 0.9 }}
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-180px)]">
                ...
                <div className="relative group">
                  <motion.div
                    ...
                    className={`w-28 h-28 rounded-full flex items-center justify-center text-4xl font-bold shadow-xl relative overflow-hidden ${
                      imagePreview ? 'border-2 border-[#171f39]' : 'bg-gradient-to-br from-[#171f39] to-[#e1ce92]'
                    }`}
                    style={
                      imagePreview
                  ...
                  >
                    <AnimatePresence>
                      {!imagePreview && (
                        ...
                        >
                          {localFormData.full_name ? localFormData.full_name.charAt(0).toUpperCase() : 'U'}
                        </motion.span>
                      )}
                    </AnimatePresence>
                  </motion.div>
              ...
              >
                {imagePreview ? 'Click camera icon to change photo' : 'Click camera icon to upload photo'}
              </motion.p>
                ...
                {getPopupFields().map((field) => {
                  ...
                  return (
                    <motion.div
                      key={field.name}
                      variants={staggerItem}
                      className={field.name === 'encrypted_bank_account' ? 'md:col-span-2' : ''}
                    >
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">
                        {field.label}
                        {!isReadOnly && <span className="text-red-500">*</span>}
                        {isReadOnly && <span className="text-gray-400 text-xs ml-1">(Read-only)</span>}
                      </label>
                      <div className="relative">
                        <motion.input
                          whileFocus={!isReadOnly ? { scale: 1.01 } : {}}
                          type={field.type}
                          name={field.name}
                        ...
                        <Icon
                          className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 ${
                            isReadOnly ? 'text-gray-500' : 'text-gray-400'
                          }`}
                        />
                      </div>
                      <AnimatePresence>
...
const AccountSettings = () => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          company_name: isDistributor ? '' : 'Tech Solutions Pvt Ltd',
          gst_number: isDistributor ? '' : '22ABCDE1234F1Z5',
          document: isDistributor ? '' : 'PAN Card',
          encrypted_aadhaar: isDistributor ? '1234-5678-9012-3456' : '',
          encrypted_pan: isDistributor ? 'ABCDE1234F' : '',
          encrypted_bank_account: isDistributor ? 'XXXXXXXXXX1234' : '',
          bank_ifsc: isDistributor ? 'HDFC0001234' : '',
          bank_holder_name: isDistributor ? 'Rahul Kumar' : ''
        });
      } catch (error) {
  ...
  const getDisplayFields = () => {
    const commonFields = [
      ...
      { name: 'phone', label: 'Phone Number', icon: Smartphone },
      { name: 'country', label: 'Country', icon: User }
    ];

    if (formData.account_type === 'distributor') {
      return [
        ...commonFields,
        { name: 'account_type', label: 'Account Type', icon: Building },
        { name: 'bank_holder_name', label: 'Bank Account Holder', icon: User },
        { name: 'encrypted_bank_account', label: 'Bank Account Number', icon: CreditCard },
        { name: 'bank_ifsc', label: 'IFSC Code', icon: Hash },
        { name: 'encrypted_pan', label: 'PAN Number', icon: FileText },
        { name: 'encrypted_aadhaar', label: 'Aadhaar Number', icon: FileText }
      ];
    }
  ...
  const navItems = [
    { id: 'profile', label: 'Profile Information', icon: UserCircle, description: 'Update your personal details' },
    { id: 'password', label: 'Change Password', icon: Key, description: 'Update your password' },
    { id: 'notifications', label: 'Notification Preferences', icon: BellDot, description: 'Manage your notifications' }
  ];
    ...
    <div className="min-h-screen bg-[#f7f2e8] p-6 rounded-2xl">
          ...
          <div className="bg-white px-8 py-6 flex items-center justify-between relative overflow-hidden border-b border-gray-100">
            ...
            <div className="flex items-center gap-4 relative z-10">
              ...
              <div>
                ...
                >
                  {formData.account_type === 'customer' ? 'Customer Profile' : 'Distributor Profile'}
                </motion.p>
              </div>
            </div>
            ...
            >
              ...
              <span className="text-gray-700 font-medium text-sm capitalize">{formData.account_type}</span>
            </motion.div>
          </div>

          {/* Navigation Cards */}
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="p-8 space-y-4">
            {navItems.map((item) => {
                    ...
                    <div>
                      <h3
                        className={`font-semibold transition-colors ${
                          isActive ? 'text-[#171f39]' : 'text-gray-800 group-hover:text-[#171f39]'
                        }`}
                      >
                        {item.label}
                      </h3>
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-sm font-medium transition-colors ${
                        isActive ? 'text-[#171f39]' : 'text-gray-400 group-hover:text-[#171f39]'
                      }`}
                    >
                      {item.id === 'profile' ? 'Edit →' : item.id === 'password' ? 'Change →' : 'Manage →'}
                    </span>
                    <motion.div
                    ...
                    >
                      <ChevronRightIcon
                        className={`w-4 h-4 transition-colors ${
                          isActive ? 'text-[#171f39]' : 'text-gray-300 group-hover:text-[#171f39]'
                        }`}
                      />
                    </motion.div>
                  </div>
                  ...
                  <motion.div
                    ...
                    style={
                      formData.profile_picture
                        ? { backgroundImage: `url(${formData.profile_picture})`, backgroundSize: 'cover', backgroundPosition: 'center' }
                        : {}
                    }
                  >
                    {!formData.profile_picture && (formData.full_name ? formData.full_name.charAt(0).toUpperCase() : 'U')}
                  </motion.div>
                  <div>
                    <h2 className="font-semibold text-gray-800">{formData.full_name || 'User'}</h2>
                    <p className="text-sm text-gray-500 capitalize">{formData.account_type}</p>
                ...
                >
                  {getDisplayFields().map((field) => {
                    ...
                    if (field.name === 'account_type') {
                      return (
                        <motion.div key={field.name} variants={staggerItem} whileHover={{ scale: 1.015 }}>
                            ...
                            <div className="absolute right-3 top-1/2 -translate-y-1/2">
                              <span
                                className={`text-xs px-2 py-1 rounded-full ${
                                  value === 'customer' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'
                                }`}
                              >
                                {value}
                              </span>
      ...
      <Notifications
        ...
        onSave={(settings) => {
          ...
          fireToast('Notification preferences saved');
        }}
      />
