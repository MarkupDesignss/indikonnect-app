...
export default function ProductDetail({ productSlug }: ProductDetailProps) {
    ...
    useEffect(() => {
        ...
        animationId = requestAnimationFrame(autoScroll);
        const pause = () => { isPaused = true; cancelAnimationFrame(animationId); };
        const resume = () => { isPaused = false; animationId = requestAnimationFrame(autoScroll); };
        ...
        <div className="overflow-hidden rounded-xl border border-[#E5E0D7] bg-[#FAF9F7]">
            <div className="grid grid-cols-1 md:grid-cols-2">
                {specRows.map(([label, value], index) => (
                    <div
                        key={label}
                        className={`flex items-center justify-between gap-5 border-b border-[#E9E5DD] px-4 py-3.5 text-[11px] ${index % 2 === 0 ? "bg-white" : "bg-[#FAF9F7]"
                            }`}
                    >
    ...
    return (
        <div className="min-h-screen bg-white font-sans text-[#17213F]">
            ...
            <main className="mx-auto w-full max-w-[1380px] px-3 pb-28 sm:px-6 sm:pb-16 lg:px-8 xl:px-10">
                {/* Breadcrumb */}
                <nav className="flex items-center gap-1.5 overflow-x-auto whitespace-nowrap py-3.5 text-[9px] text-[#8A92A3] sm:gap-2 sm:py-4 sm:text-[11px] hide-scrollbar">
                ...
                {/* Product Hero */}
                <div className="grid grid-cols-1 gap-7 lg:grid-cols-[minmax(0,1.08fr)_minmax(420px,.92fr)] lg:items-start lg:gap-8 xl:gap-14">
                    {/* Left Gallery */}
                    <div className="min-w-0 lg:sticky lg:top-4 lg:self-start">
                        <div className="flex flex-col gap-3 sm:gap-4 lg:grid lg:grid-cols-[76px_minmax(0,1fr)] xl:grid-cols-[86px_minmax(0,1fr)]">
                            {/* Desktop Thumbnails */}
                            <div className="hidden max-h-[650px] flex-col gap-3 overflow-y-auto pr-1 lg:flex hide-scrollbar">
                                {gallery.map((img: string, index) => (
                                    <button
                                        key={index}
                                        type="button"
                                        onClick={() => setActiveImage(index)}
                                        className={`group relative h-[76px] w-[76px] flex-shrink-0 overflow-hidden rounded-xl border bg-[#F8F6F1] transition-all xl:h-[82px] xl:w-[82px] ${activeImage === index
                            ...
                            {/* Mobile Thumbnails */}
                            <div className="order-1 flex w-full gap-2.5 overflow-x-auto pb-1 lg:hidden hide-scrollbar">
                                {gallery.map((img: string, index) => (
                                    <button
                                        key={index}
                                        type="button"
                                        onClick={() => setActiveImage(index)}
                                        className={`relative h-[68px] w-[68px] flex-shrink-0 overflow-hidden rounded-xl border-2 bg-[#F8F6F1] sm:h-[76px] sm:w-[76px] ${activeImage === index
                            ...
                            <div
                                ref={mainImageBoxRef}
                                className="order-2 relative mx-auto aspect-square w-full min-w-0 max-w-[620px] cursor-zoom-in overflow-hidden rounded-[18px] border border-[#E9E4DA] bg-[#F7F4EE] sm:rounded-[22px] lg:order-none lg:mx-0 lg:max-w-none"
                            ...
                            >
                                ...
                                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 rounded-full bg-black/50 px-3 py-1.5 text-[8px] text-white backdrop-blur-sm sm:bottom-4 sm:text-[9px]">
                                    Tap to zoom
                                </div>

                                {product.rating >= 4.5 && (
                                    <span className="absolute left-3 top-3 rounded-full bg-[#071A41] px-3 py-1.5 text-[8px] font-bold uppercase tracking-[0.15em] text-white shadow-lg sm:left-4 sm:top-4 sm:px-4 sm:py-2 sm:text-[9px]">
                                        Bestseller
                                    </span>
                                )}

                                {product.discount && product.discount > 0 && (
                                    <span className="absolute bottom-3 left-3 rounded-full bg-[#071A41]/80 px-2.5 py-1 text-[9px] font-bold text-white backdrop-blur-sm sm:bottom-4 sm:left-4 sm:px-3 sm:py-1.5 sm:text-[10px]">
                                ...
                                <button
                                    ...
                                    className="absolute right-3 top-3 flex h-10 w-10 items-center justify-center rounded-full border border-white/60 bg-white/95 shadow-lg backdrop-blur transition hover:scale-105 sm:right-4 sm:top-4 sm:h-11 sm:w-11"
                                >
                                    <Heart className={`h-[17px] w-[17px] sm:h-[18px] sm:w-[18px] ${isWishlisted ? "fill-red-500 text-red-500" : "text-[#071A41]"}`} />
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Right Product Info */}
                    <div className="min-w-0 lg:max-h-[calc(100vh-120px)] lg:overflow-y-auto lg:pr-2 lg:scroll-smooth hide-scrollbar">
                        <div className="mb-2.5 text-[9px] font-bold uppercase tracking-[0.22em] text-[#B0832F] sm:mb-3 sm:text-[10px]">
                            {product.category || "IndieKonnect Atelier"}
                        </div>

                        <h1 className={`${marcellus.className} max-w-[620px] text-[27px] leading-[1.12] text-[#071A41] sm:text-[36px] xl:text-[40px]`}>
                            {product.name}
                        </h1>
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2 sm:mt-4 sm:gap-2.5">
                            <div className="flex items-center gap-0.5">
                                {[...Array(5)].map((_, index) => (
                                    <Star
                                        key={index}
                                        className={`h-[15px] w-[15px] ${index < Math.floor(product.rating)
                            ...
                            <span className="text-[12px] text-black sm:text-[13px]">
                                {product.rating.toFixed(1)} · {product.reviews?.toLocaleString() || 0} reviews
                            </span>
                            {userAccountType === "distributor" && (
                                <span className="rounded-full bg-[#E8F4FD] px-2.5 py-1 text-[8px] font-bold text-[#0A5C8A] sm:ml-1 sm:text-[9px]">
                                    Distributor
                                </span>
                            )}
                        </div>
                        ...
                        {/* Price */}
                        <div className="mt-5 border-y border-[#E8E3D9] py-4 sm:mt-6 sm:py-5">
                            <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
                                <span className="text-[28px] font-semibold tracking-[-0.03em] text-[#071A41] sm:text-[32px]">
                                    ₹{product.price.toLocaleString()}
                                </span>
                                {product.mrp > 0 && product.mrp > product.price && (
                                    <>
                                        <span className="text-[12px] text-[#9298A4] line-through sm:text-[14px]">
                                            MRP: ₹{product.mrp.toLocaleString()}
                                        </span>
                                        {product.discount && product.discount > 0 && (
                                            <span className="rounded-full bg-[#EDF7F0] px-2.5 py-1 text-[9px] font-bold text-[#16713C] sm:text-[10px]">
                                                {product.discount}% OFF
                                            </span>
                                        )}
                                    </>
                                ...
                                {product.isDealOfTheDay && product.isActiveDeal && (
                                    <span className="rounded-full bg-[#FF6B35] px-2.5 py-1 text-[8px] font-bold text-white animate-pulse">
                                        🔥 Deal of the Day
                                    </span>
                                )}
                            </div>
                            <p className="mt-1.5 text-[11px] text-black/80 sm:text-[12px]">
                                Inclusive of all taxes · Free shipping over ₹999
                            </p>
                        </div>

                        {/* Product Meta */}
                        <div className="mt-4 flex flex-wrap items-center gap-2.5 text-[9px] text-[#697286] sm:mt-5 sm:text-[10px]">
                            {product.productCode && (
                                <>
                        ...
                        <div className="mt-5 space-y-5 sm:mt-6 sm:space-y-6">
                            ...
                            <div>
                                <div className="mb-2.5 flex items-center justify-between sm:mb-3">
                                    <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-[#667085] sm:text-[10px]">
                                    ...
                                    {colorOptions.map((color) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`relative flex h-8 w-8 items-center justify-center rounded-full transition-all ${isSelected ? "scale-105" : "hover:scale-105"
                                                    }`}
                                            >
                                                <span
                                                ...
                                                />
                                                {isSelected && <span className="absolute inset-[-3px] rounded-full border border-[#C89B3C]" />}
                                            </button>
                                        );
                                    })}
                                </div>
                            ...
                            <div>
                                <div className="mb-2.5 flex items-center justify-between sm:mb-3">
                                    ...
                                    <button type="button" className="text-[9px] font-semibold text-[#071A41] underline underline-offset-2 transition hover:text-[#B0832F] sm:text-[10px]">
                                        Size guide
                                    </button>
                                </div>
                                    ...
                                    {sizeOptions.map((size) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`flex h-9 min-w-[40px] items-center justify-center rounded-lg border px-3 text-[10px] font-semibold transition-all sm:h-10 sm:min-w-[42px] sm:text-[11px] ${isSelected
                        ...
                        <div className="mt-6 hidden items-center gap-4 sm:flex">
                            <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#667085]">Quantity</span>
                            <div className="flex h-10 items-center overflow-hidden rounded-full border border-[#DCD8D0] bg-white">
                                ...
                                <span className="w-9 text-center text-[12px] font-semibold text-[#071A41]">{quantity}</span>
                                <button
                                    type="button"
                        ...
                        {/* Trust Strip */}
                        <div className="mt-5 grid grid-cols-1 gap-2 py-4 sm:mt-6 sm:grid-cols-3">
                            <div className="flex items-center gap-3 rounded-xl border border-[#E8E3D9] bg-white px-3 py-2 shadow-sm transition hover:shadow-md">
                                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-[#C89B3C]/20 to-[#C89B3C]/5">
                                    <RotateCcw className="h-4 w-4 text-[#C89B3C]" />
                                </div>
                                <div>
                                    <p className="text-[10px] font-semibold text-[#071A41]">30-Day Returns</p>
                                    <p className="text-[8px] text-[#8790A1]">Hassle-free returns</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 rounded-xl border border-[#E8E3D9] bg-white px-3 py-2 shadow-sm transition hover:shadow-md">
                                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-[#C89B3C]/20 to-[#C89B3C]/5">
                                ...
                                <div>
                                    <p className="text-[10px] font-semibold text-[#071A41]">Secure Checkout</p>
                                    <p className="text-[8px] text-[#8790A1]">Protected payments</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 rounded-xl border border-[#E8E3D9] bg-white px-3 py-2 shadow-sm transition hover:shadow-md">
                                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-[#C89B3C]/20 to-[#C89B3C]/5">
                                    <BadgeCheck className="h-4 w-4 text-[#C89B3C]" />
                                </div>
                                <div>
                                    <p className="text-[10px] font-semibold text-[#071A41]">Quality Crafted</p>
                                    <p className="text-[8px] text-[#8790A1]">Handmade with love</p>
                                </div>
                            </div>
                        </div>

                        {/* Collapsible */}
                        <div className="mt-4 border-t border-[#E5E0D7] sm:mt-6">
                            {/* Description */}
                            <div className="border-b border-[#E5E0D7]">
                                <button
                                    type="button"
                                    onClick={() => setActiveTab(activeTab === "description" ? "" : "description")}
                                    className="flex w-full items-center justify-between py-4 text-left sm:py-[17px]"
                                >
                                    <span className="text-[11px] font-semibold text-[#071A41] sm:text-[12px]">Description</span>
                                ...
                                <AnimatePresence initial>
                                    {activeTab === "description" && (
                                        ...
                                        >
                                            <div className="pb-6">
                                                <p className="whitespace-pre-wrap text-[11px] leading-6 text-[#69758A] sm:text-[12px]">
                                                ...
                                                <div className="mt-5 space-y-3">
                                                    {["Premium quality material", "Ergonomic design for maximum comfort", "Sturdy construction & finish", "Perfect for living room, bedroom or office"].map((feature) => (
                                                        <div key={feature} className="flex items-center gap-2.5 text-[10px] text-[#526076] sm:text-[11px]">
                            ...
                            <div className="border-b border-[#E5E0D7]">
                                <button
                                    ...
                                    className="flex w-full items-center justify-between py-4 text-left sm:py-[17px]"
                                >
                                    <span className="text-[11px] font-semibold text-[#071A41] sm:text-[12px]">Specifications</span>
                                ...
                                <AnimatePresence initial={false}>
                                    {activeTab === "specifications" && (
                                        ...
                                        >
                                            <div className="pb-7">
                                                <div className="mb-2 text-[13px] font-semibold text-[#071A41] sm:text-[14px]">Product Specifications</div>
                                                <p className="mb-4 text-[9px] text-[#8992A3] sm:text-[10px]">Product details and specifications</p>
                                                {renderSpecTable()}
                                            </div>
                                        </motion.div>
                                    )}
                            ...
                            <div className="border-b border-[#E5E0D7]">
                                <button
                                    ...
                                    className="flex w-full items-center justify-between py-4 text-left sm:py-[17px]"
                                >
                                    <span className="text-[11px] font-semibold text-[#071A41] sm:text-[12px]">Shipping & Returns</span>
                                    {activeTab === "shipping" ? (
                                        <ChevronDown className="h-4 w-4 text-[#B0832F]" />
                                    ) : (
                                        <ChevronRight className="h-4 w-4 text-[#8B94A5]" />
                                        ...
                                        >
                                            <div className="grid grid-cols-1 gap-3 pb-6 sm:grid-cols-3">
                                                <div className="rounded-xl border border-[#E5E0D7] bg-[#FAF9F7] p-4 text-center">
                                                    ...
                                                    <h3 className="mt-2 text-[11px] font-semibold text-[#071A41]">Free Shipping</h3>
                                                    <p className="mt-1 text-[9px] text-[#697286]">On orders above ₹999</p>
                                                </div>
                                                <div className="rounded-xl border border-[#E5E0D7] bg-[#FAF9F7] p-4 text-center">
                                                    <RotateCcw className="mx-auto h-5 w-5 text-[#C89B3C]" />
                                                    <h3 className="mt-2 text-[11px] font-semibold text-[#071A41]">Easy Returns</h3>
                                                    <p className="mt-1 text-[9px] text-[#697286]">7 days return policy</p>
                                                </div>
                                                <div className="rounded-xl border border-[#E5E0D7] bg-[#FAF9F7] p-4 text-center">
                                                    <Shield className="mx-auto h-5 w-5 text-[#C89B3C]" />
                                                    <h3 className="mt-2 text-[11px] font-semibold text-[#071A41]">Secure Checkout</h3>
                                                    <p className="mt-1 text-[9px] text-[#697286]">Safe and secure payments</p>
                                                </div>
                                            </div>
                                        </motion.div>
                                    )}
                            ...
                            <div className="border-b border-[#E5E0D7]">
                                <button
                                    ...
                                    className="flex w-full items-center justify-between py-4 text-left sm:py-[17px]"
                                >
                                    <span className="text-[11px] font-semibold text-[#071A41] sm:text-[12px]">
                                                ...
                                                {product.reviews > 0 ? (
                                                    <>
                                                        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                                                            <div>
                                                                <h3 className="text-[14px] font-semibold text-[#071A41] sm:text-[15px]">Customer Reviews</h3>
                                                                <p className="mt-1 text-[9px] text-[#8992A3] sm:text-[10px]">Based on {product.reviews} reviews</p>
                                                            </div>
                                                        ...
                                                        {ratingBreakdown.some((r) => r.pct > 0) && (
                                                            <div className="mt-6 space-y-3">
                                                                {ratingBreakdown.map((rating) => rating.pct > 0 && (
                                                                    <div key={rating.stars} className="flex items-center gap-2 text-[10px]">
                                                                        <span className="w-11 text-[#596275]">{rating.stars} star</span>
                                                                        ...
                                                                        <span className="w-8 text-right text-[#8992A3]">{rating.pct}%</span>
                                                                    </div>
                                                                ))}
                                                            </div>
                                                        )}
                                                        <div className="mt-6 space-y-4">
                                                            <div className="rounded-xl border border-[#E5E0D7] bg-white p-4">
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <span className="text-[10px] text-[#8992A3]">Verified Purchase</span>
                                                                </div>
                                                                <p className="mt-1.5 text-[11px] text-[#69758A]">
                                                                    Great product! Highly recommend. Quality is excellent and exactly as described.
                                                                </p>
                                                                <p className="mt-1 text-[9px] text-[#8992A3]">- Customer</p>
                                                            </div>
                                                        </div>
                                                    </>
                                                ) : (
                                                    <>
                                                        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                                                            <div>
                                                                <h3 className="text-[14px] font-semibold text-[#071A41] sm:text-[15px]">Customer Reviews</h3>
                                                                <p className="mt-1 text-[9px] text-[#8992A3] sm:text-[10px]">Based on 0 reviews</p>
                                                            </div>
                                                            <div className="flex items-center gap-2">
                                                                <div className="flex items-center gap-0.5">
                                                                    {[...Array(5)].map((_, index) => (
                                                                        <Star key={index} className="h-3.5 w-3.5 fill-[#ECECEC] text-[#ECECEC]" />
                                                                    ))}
                                                                </div>
                                                                <span className="text-[11px] font-semibold text-[#071A41]">0.0 / 5</span>
                                                            </div>
                                                        </div>
                                                        <div className="mt-6 space-y-3">
                                                            {[5, 4, 3, 2, 1].map((rating) => (
                                                                <div key={rating} className="flex items-center gap-2 text-[10px]">
                                                                    <span className="w-11 text-[#596275]">{rating} star</span>
                                                                    <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[#ECEAE6]">
                                                                        <div className="h-full rounded-full bg-[#C89B3C]" style={{ width: "0%" }} />
                                                                    </div>
                                                                    <span className="w-8 text-right text-[#8992A3]">0%</span>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    </>
                ...
                {similarProducts.length > 0 && (
                    <section className="mt-12 sm:mt-14 lg:mt-20">
                        <div className="mb-7 text-center sm:mb-8">
                            <p className="mb-2 text-[8px] font-bold uppercase tracking-[0.25em] text-[#B0832F] sm:text-[9px]">Curated for you</p>
                            <h2 className={`${marcellus.className} text-2xl text-[#071A41] md:text-3xl`}>You may also like</h2>
                            <div className="mx-auto mt-3 h-px w-12 bg-[#C89B3C]" />
                        </div>
                        <div ref={sliderRef} className="grid grid-cols-2 gap-x-3 gap-y-6 overflow-x-auto pb-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 hide-scrollbar">
                                ...
                                >
                                    <div className="relative aspect-square overflow-hidden rounded-xl border border-[#E8E3D9] bg-[#F7F4EE]">
                                        ...
                                        <button
                                            ...
                                            aria-label="Add to wishlist"
                                        >
                                            <Heart className={`h-4 w-4 ${wishlistState[similarProduct.id] ? "fill-red-500 text-red-500" : "text-[#071A41]"}`} />
                                        </button>
                                        ...
                                        {!similarProduct.inStock && (
                                            <div className="absolute inset-0 flex items-center justify-center bg-[#071A41]/20">
                                                <span className="rounded-full bg-white px-3 py-1 text-[9px] font-semibold text-[#071A41]">Out of Stock</span>
                                            </div>
                                        )}
                                    </div>
                                    <div className="mt-3 text-center">
                                        <h3 className="line-clamp-1 text-[11px] font-medium text-[#343B4A] sm:text-[12px]">{similarProduct.name}</h3>
                                        <div className="mt-1 flex items-center justify-center gap-2">
                                            <span className="text-[13px] font-semibold text-[#071A41] sm:text-[14px]">₹{similarProduct.price.toLocaleString()}</span>
                                            {similarProduct.originalPrice > 0 && similarProduct.originalPrice > similarProduct.price && (
                                                <span className="text-[9px] text-[#9AA0AA] line-through sm:text-[10px]">₹{similarProduct.originalPrice.toLocaleString()}</span>
                                            )}
                                        </div>
                                        <div className="mt-1 flex items-center justify-center gap-1">
                                            <span className="text-[9px] tracking-wide text-[#C89B3C] sm:text-[10px]">{renderRatingStars(similarProduct.rating)}</span>
                                            <span className="text-[8px] text-[#9AA0AA] sm:text-[9px]">({similarProduct.reviews || 0})</span>
                                        </div>
                                    </div>
                                </Link>
                            ))}
            ...
            {/* Mobile Sticky Bar */}
            <div className="fixed inset-x-0 bottom-0 z-[90] border-t border-[#E5E0D7] bg-white/95 px-2.5 pb-[max(10px,env(safe-area-inset-bottom))] pt-2.5 shadow-[0_-8px_30px_rgba(7,26,65,0.08)] backdrop-blur-xl sm:hidden">
                <div className="flex items-center gap-2">
                    <div className="flex h-[46px] flex-shrink-0 items-center overflow-hidden rounded-full border border-[#DCD8D0] bg-white">
                        <button
                            type="button"
                            onClick={() => handleQuantityChange("decrement")}
                            disabled={quantity <= 1}
                            className="flex h-full w-8 items-center justify-center text-[#071A41] disabled:opacity-30"
                            aria-label="Decrease quantity"
                        >
                            <Minus className="h-3 w-3" />
                        </button>
                        <span className="w-6 text-center text-[11px] font-semibold text-[#071A41]">{quantity}</span>
                        <button
                            type="button"
                            onClick={() => handleQuantityChange("increment")}
                            disabled={quantity >= 10 || quantity >= (product.stockQuantity || 10)}
                    ...
                    <motion.button
                        ...
                        disabled={!product.inStock}
                        className={`ik-glass flex h-[46px] min-w-0 flex-1 items-center justify-center gap-1.5 rounded-full px-3 text-[9px] font-bold uppercase tracking-[0.05em] ${product.inStock
                    ...
                    <motion.button
                        ...
                        onClick={(e) => { fireRipple(e); handleBuyNow(); }}
                        disabled={!product.inStock}
                        className={`flex h-[46px] flex-1 items-center justify-center rounded-full border px-3 text-[9px] font-bold uppercase tracking-[0.05em] ${product.inStock
                        ...
                        <div className="absolute bottom-4 left-1/2 flex max-w-[80vw] -translate-x-1/2 gap-2 overflow-x-auto px-4">
                            {gallery.map((img: string, index) => (
                                <button
                                    ...
                                    className={`relative h-12 w-12 flex-shrink-0 overflow-hidden rounded border-2 transition ${index === fullscreenImageIndex ? "border-[#C89B3C]" : "border-transparent opacity-50 hover:opacity-80"
                                        }`}
                                >
        ...
        @media (max-width: 1023px) {
          .lg\\:max-h-\\[calc\\(100vh-120px\\)\\] { max-height: none !important; overflow-y: visible !important; }
        }
